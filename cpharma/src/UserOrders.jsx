...
function OrderList(props) {  
  ...
  useEffect(() => {  
    const GetData = async () => {  
      const result = await axios('http://localhost:46152/api/Order');  
      setData(result.data);  
    };  
  ...
  useEffect(() =>{
    (
    async () =>{
      const response =  await fetch('http://localhost:46152/api/Auth/User',{
                method:'GET',
  ...
  const deleteorder = (orderId) => {  
    axios.delete('http://localhost:46152/api/Order/' +orderId)  
      .then((result) => {  
